...
final readonly class AdminProductResultDto
{
    public static function fromProduct(Product $product): self
    {
        ...
        if ($product->relationLoaded('category')) {
            ...
            if ($loadedCategory instanceof Category) {
                $category = AdminProductCategoryResultDto::fromCategory($loadedCategory);
            }
        }

        /** @var list<AdminProductVariantResultDto> $variants */
        $variants = [];
            ...
            if ($loadedVariants instanceof Collection) {
                foreach ($loadedVariants as $variant) {
                    if ($variant instanceof ProductVariant) {
                        $variants[] = AdminProductVariantResultDto::fromVariant($variant);
                    }
                }
            }
        }
        ...
        return new self(
            ...
            variants: $variants,
            publishedAt: self::formatAtomDate($product->published_at),
            createdAt: self::formatAtomDate($product->created_at),
            updatedAt: self::formatAtomDate($product->updated_at),
        );
    }

    /**
     * @param  list<AdminProductVariantResultDto>  $variants
     */
    public function __construct(
        ...
        public ?AdminProductCategoryResultDto $category,
    ...
    public function toArray(): array
    {
        return [
            ...
            'variants' => array_map(
                static fn (AdminProductVariantResultDto $variant): array => $variant->toArray(),
                $this->variants
            ),
