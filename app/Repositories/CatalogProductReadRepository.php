...
final class CatalogProductReadRepository
{
    ...
    private function applyFilters(Builder $query, CatalogProductListFilterDto $filter): void
    {
        ...
        match ($sort) {
            'price_asc' => $query->orderByRaw('(variants_min_price + 0) asc')->orderBy('id'),
            'price_desc' => $query->orderByRaw('(variants_min_price + 0) desc')->orderByDesc('id'),
