...
Last revised: `2026-07-05` (`C4` closure)
...
### Wave C3 — Closed (`2026-07-05`) — Cart Module

Move cart transport, `Application/Cart/*`, `Services/Cart/*` (resolver, mutation, mapper), `CartPolicy`, and cart contracts into `app/Domains/Cart`, preserving ownership-guard and locking semantics. No cart repositories exist (mutation service hits Eloquent directly).

DoD: cart routes serve identical contracts from the module namespace; cross-module callers (Users, Checkout, smoke) import cart contracts through `App\Domains\Cart\Contracts\*`; guardrails updated; old namespaces removed in the same block; `CartServiceProvider` registered in `bootstrap/providers.php`. **Met on `2026-07-05`.**

Closed scope (`2026-07-05`):
- 1 controller + 2 FormRequests relocated to `app/Domains/Cart/Controllers/`; all 3 made `final` on move.
- Application layer relocated (`Commands/`, `Queries/`, `Dto/`) covering 2 command handlers + 2 readonly payloads + 1 query handler + 1 query payload + 5 DTOs.
- 4 services relocated to `app/Domains/Cart/Services/` (`CartService`, `CartResolver`, `CartMutationService`, `CartResultMapper`).
- 2 contracts relocated to `app/Domains/Cart/Contracts/` (`CartServiceInterface`, `CartMutationServiceInterface`); preserved existing interface names (no rename to `CartService`) for minimal-diff at 6+ cross-module call sites. `CartServiceInterface` is the cross-module surface (consumed by Users `LoginAuthUserHandler`, Checkout orchestrator, smoke scenarios); `CartMutationServiceInterface` is module-internal (consumed by `CartService` + tests).
- `CartPolicy` relocated to `app/Domains/Cart/Policies/`; made `final`; `Gate::policy(Cart::class, CartPolicy::class)` registration moved from `AppServiceProvider::boot()` to `CartServiceProvider::boot()`.
- New `CartServiceProvider` registered in `bootstrap/providers.php`; binds the 2 contracts and owns the policy registration. `ApplicationBindingsServiceProvider` shrunk (2 cart bindings + 4 imports removed). `AppServiceProvider` shrunk (cart model + policy imports + `Gate::policy` line removed).
- Cross-module callers rewired from `App\Contracts\CartServiceInterface` / `App\Contracts\CartMutationServiceInterface` to `App\Domains\Cart\Contracts\*`:
  - `app/Domains/Users/Application/Commands/LoginAuthUserHandler.php` — first real cross-module `Domains\<Other>\Contracts\` import (Users → Cart).
  - `app/Services/Checkout/CheckoutPlaceOrderOrchestrator.php` — stays in legacy bridge until C4.
...
- Operational contracts preserved byte-identical: HTTP wire (3 endpoints), `CartStatus` enum literal values (`active`, `checked_out`, `abandoned`), guest cart token resolution channels (`?guest_token=` query OR `X-Cart-Token` header), `CartException` literal messages, locking semantics (`lockForUpdate` within `DB::transaction`), ownership assertions (`assertOwnership`).
- Guardrails updated: `PolicyCompletenessMatrixGuardrailTest` (CartPolicy FQCN), `InfrastructureProviderBoundaryTest` (provider list). `ModuleBoundaryGuardrailTest` passes with the first real cross-module `Domains\<Other>\Contracts\` import (Users → Cart).
- 2 unit tests relocated to `tests/Unit/Application/Cart/` (`CartResultMapperTest`, `CartResolverTest`); `CartModuleRelocationTest` added (4 tests — 2 contract bindings, controller namespace via route resolution, `Gate::policy()` registration, cross-module resolution from Users handler).
- `psalm.xml` suppressions extended for the module services and DTO directories.
- `app/Domains/Cart/README.md` replaced with the active module documentation (subfolders, contract surface with cross-module consumers, operational contracts, migration state).
- Shared concerns stay in legacy bridge per C0: `ResolvesAuthenticatedUser` trait (used cross-module by Users and Checkout controllers), Eloquent models (`Cart`, `CartItem`), `CartStatus` enum (heavy cross-module usage), `CartException` (shared kernel).


### Wave C4 — Closed (`2026-07-05`) — Checkout Module
...
DoD: checkout routes serve identical contracts from the module namespace; cross-module callers (smoke scenarios, smoke infra) import checkout contracts through `App\Domains\Checkout\Contracts\*`; guardrails updated; old namespaces removed in the same block; `CheckoutServiceProvider` registered in `bootstrap/providers.php`. **Met on `2026-07-05`.**

Closed scope (`2026-07-05`):
- 1 controller + 2 FormRequests relocated to `app/Domains/Checkout/Controllers/`; controller + `PlaceOrderRequest` made `final` on move (`InitiatePaymentRequest` already final).
- Application layer relocated: 2 command handlers + 2 readonly payloads (`Commands/`), 12 DTOs (`Dto/`).
- 9 services relocated to `app/Domains/Checkout/Services/`: `CheckoutService` (implements `CheckoutServiceInterface`), `CheckoutPlaceOrderOrchestrator` (already imports `App\Domains\Cart\Contracts\CartServiceInterface` per C3; intra-module checkout contract rewired), `CheckoutOrderWriter`, `CheckoutOrderFinalizer`, `CheckoutIdempotencyGuard`, `CheckoutDiscountResolver`, `FreeCheckoutShippingCostResolver`, `CheckoutInventoryAllocator`, `CheckoutCartPreparer`, `CheckoutRequestIdentityResolver`. 4 service DTOs relocated to `Services/Dto/`.
- 2 contracts relocated to `app/Domains/Checkout/Contracts/`:
  - `CheckoutServiceInterface` (cross-module; 1 method `placeOrder`).
  - `CheckoutShippingCostResolver` (promoted from interface-in-services pattern; module-internal extension point; default `FreeCheckoutShippingCostResolver`).
- `EnsureIdempotencyKeyMiddleware` relocated to `app/Domains/Checkout/Middleware/`; alias `idempotency.key` preserved; FQCN updated in `bootstrap/app.php`.
- New `CheckoutServiceProvider` registered in `bootstrap/providers.php` (after `CartServiceProvider`, before `GatewayServiceProvider`); binds the 2 contracts and owns `RateLimiter::for('checkout', ...)` in `boot()` (relocated from `AppServiceProvider::boot()`). `ApplicationBindingsServiceProvider` shrunk (2 checkout bindings + 3 imports removed). `AppServiceProvider` shrunk (checkout rate limiter removed).
- Cross-module callers rewired from `App\Contracts\CheckoutServiceInterface` / `App\Application\Checkout\Dto\*` to `App\Domains\Checkout\Contracts\*` / `App\Domains\Checkout\Application\Dto\*`: `CheckoutPlaceOrderPerformanceSmokeScenario`, `WebhookFlowScenario`, `PerformanceSmokeSetupFactory`, `PerformanceSmokeContextDto`.
- Operational contracts preserved byte-identical:
  - HTTP wire (2 endpoints: `POST /checkout/place-order` returns `201` with order+payment envelope; `POST /checkout/orders/{order}/pay` returns order+payment envelope) — paths, verbs, request schemas, response schemas, status codes, middleware (`active.api.user`, `throttle:checkout`, `idempotency.key`).
  - Idempotency semantics (`Idempotency-Key` header requirement; 5-branch guard resolution; scope_key construction; retention windows `pending_minutes=30`/`completed_hours=24`; payload-mismatch behavior).
  - Rate limiting (`throttle:checkout` = 6/min by user-id or IP).
  - Money semantics (decimal:2 precision; R2 promotion arithmetic locked by `CheckoutIdempotencyAndPromotionArithmeticGuardrailTest`).
  - Address payload boundary (`{line1, city, country, postcode}` closed shape locked by `AddressPayloadBoundaryGuardrailTest`).
  - Initial state tuple (`Order{status: pending, payment_status: pending, shipment_status: pending}` via `forceFill` — NOT mass assignment).
  - Locking semantics (`Cart`/`CheckoutIdempotency`/`Inventory`/`Coupon`/`Promotion` `lockForUpdate()` within `DB::transaction()` in `CheckoutService::placeOrder`).
  - Event dispatch (`OrderPlaced` via `event()` in `CheckoutOrderFinalizer::finalize()`; implements `ShouldDispatchAfterCommit`; listeners dispatch via `->afterCommit()`).
  - Cart transition (`CartStatus::CHECKED_OUT` via `$cart->update(...)` — Cart status is fillable, unlike Order/Payment/Shipment).
- Guardrails updated: `ApplicationCheckoutCommandBoundaryTest` (discovery path + namespace literal), `CheckoutIdempotencyAndPromotionArithmeticGuardrailTest` (4 literal paths), `AddressPayloadBoundaryGuardrailTest` (DTO import + 3 construction-site paths), `LegacyPayloadArtifactGuardrailTest` (namespace literal), `InfrastructureProviderBoundaryTest` (provider list).
- 5 unit tests relocated to `tests/Unit/Application/Checkout/` (`CheckoutDiscountResolverTest`, `CheckoutIdempotencyRetentionConfigTest`, `CheckoutOrderFinalizerTest`, `CheckoutRequestIdentityResolverTest`, `CheckoutShippingCostResolverBindingTest`); `CheckoutModuleRelocationTest` added (4 tests — 2 contract bindings, controller namespace via route resolution for both endpoints, `idempotency.key` middleware alias FQCN, `throttle:checkout` rate limiter registration).
- `psalm.xml` suppressions extended for the module services + DTO directories.
- `app/Domains/Checkout/README.md` replaced with the active module documentation (subfolders, contract surface with cross-module consumers, operational contracts — wire + idempotency + rate limiting + money + address boundary + initial state + locking + event dispatch + cart transition, out-of-scope items for C5/C6/C7, migration state).
- Shared concerns stay in legacy bridge per C0:
  - `ResolvesAuthenticatedUser` trait (used cross-module by Users, Cart, and Checkout controllers).
  - Eloquent models (`Order`, `OrderItem`, `Payment`, `Shipment`, `CheckoutIdempotency`).
  - Enums (`OrderStatus`, `PaymentStatus`, `ShipmentStatus` — heavy cross-module usage).
  - Events (`OrderPlaced` + status-changed events; `OrderPlaced` emitted by checkout, listened cross-module by `QueueOrderSideEffects`).
  - `CheckoutException` (shared kernel).
  - `OrderPaymentStatusResolver` (shared kernel; used cross-module by listeners + `DispatchShipmentJob`).
  - `OrderPolicy` (stays for C5 — authorizes `Order` model, owned by Orders module).
  - Payment/shipping services (`PaymentService`, `ShippingService`) — consumed via legacy bridge until C6/C7.
...
| `2026-07-05` | `C3` closed: third runtime module move executed. Cart bounded context relocated into `app/Domains/Cart/*` with namespace moves and zero dual-namespace shims — `Controllers/` (1 controller — `CartController` made `final` on move; + 2 FormRequests, both made `final`); `Application/Commands/` (2 handlers + 2 readonly payloads), `Application/Queries/` (1 handler + 1 payload), `Application/Dto/` (5 DTOs); `Services/` (4 implementations — `CartService`, `CartResolver`, `CartMutationService`, `CartResultMapper`); `Contracts/` (2 interfaces — `CartServiceInterface` cross-module, `CartMutationServiceInterface` module-internal; preserved existing interface names for minimal-diff at 6+ cross-module call sites); `Policies/CartPolicy` (made `final`). New `CartServiceProvider` registered in `bootstrap/providers.php`; binds the 2 contracts and registers `Gate::policy(Cart::class, CartPolicy::class)` in `boot()`. Atomic wiring — `routes/api.php` controller import switched; `ApplicationBindingsServiceProvider` cart bindings removed (2 bindings + 4 imports); `AppServiceProvider` cart policy registration removed (model + policy imports + `Gate::policy` line); `psalm.xml` per-directory suppressions extended for `app/Domains/Cart/Services` and `app/Domains/Cart/Application/Dto`. Cross-module callers rewired from `App\Contracts\CartServiceInterface`/`CartMutationServiceInterface` to `App\Domains\Cart\Contracts\*` — `LoginAuthUserHandler` (Users module; first real cross-module `Domains\<Other>\Contracts\` import exercised by `ModuleBoundaryGuardrailTest`), `CheckoutPlaceOrderOrchestrator` (stays in legacy bridge until C4), 4 smoke scenarios / infra files, 3 test files. Tests — 2 unit tests relocated to `tests/Unit/Application/Cart/` (`CartResultMapperTest`, `CartResolverTest`); `CartModuleRelocationTest` (new, 4 tests) locks the 2 contract bindings + controller namespace via route resolution + `Gate::policy()` registration + cross-module resolution path; `PolicyCompletenessMatrixGuardrailTest` CartPolicy FQCN updated; `InfrastructureProviderBoundaryTest` provider list updated. Operational contracts preserved byte-identical — HTTP wire (3 endpoints: `GET /cart`, `POST /cart/items`, `DELETE /cart/items/{variantId}`), `CartStatus` enum literal values (`active`, `checked_out`, `abandoned`), guest cart token resolution channels (`?guest_token=` query OR `X-Cart-Token` header), `CartException` literal messages mapped by `ApiExceptionRenderer`, locking semantics (`lockForUpdate` on `Cart`/`CartItem`/`User` rows within `DB::transaction`), ownership assertions (`CartMutationService::assertOwnership`); `/api/v1/cart/*` HTTP contracts locked by `docs/api/openapi.yaml` (S1) and verified by `OpenApiConformanceFeatureTest`. Shared `ResolvesAuthenticatedUser` trait stays at `app/Http/Controllers/Concerns/` (still used cross-module by Users and Checkout); Eloquent models (`Cart`, `CartItem`), `CartStatus` enum (heavy cross-module usage: checkout, maintenance, admin, factories), and `CartException` (shared kernel) stay shared under their respective legacy-bridge allowances pending the post-C7 model-ownership wave. Frontend untouched (URL-only coupling). The C0 boundary contract remains green; the legacy-bridge allowlist effectively shrinks further (`App\Contracts\CartServiceInterface`/`CartMutationServiceInterface` and `App\Services\Cart\*` no longer in legacy paths); first real cross-module `Domains\<Other>\Contracts\` import validated end-to-end (Users → Cart); `C4-C7` are the remaining roadmap surface |
