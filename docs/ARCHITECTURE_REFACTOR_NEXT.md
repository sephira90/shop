...
Last revised: `2026-07-04` (`Q3` closure)
...
- The active block is `Q4` (frontend type/test hardening); the full order is fixed in the Execution Queue below.
...
| 12 | `Q4` Frontend type/test hardening | P2 | Active — next up |
...
23. Frontend hardening flags enabled; per-run coverage signal visible in CI — `Q4`.
24. Covered `/api/v1` routes are validated against a machine-readable spec in CI — `S1`.
