...
Last revised: `2026-07-05` (`C1` closure)
...
26. ~~First module slices (Catalog, Users) serving production traffic from `app/Domains/*` — `C1-C2`.~~ — partial close `2026-07-05`. `C1` (Catalog) achieved: public read slice lives in `app/Domains/Catalog/*` with contract surface `CatalogProductReadRepository` + `CatalogCacheVersion` + `CatalogReadService` + `Dto/CatalogProductListFilterDto`; admin writes consume `CatalogCacheVersion`; performance-smoke scenarios consume `CatalogReadService`; `CatalogServiceProvider` registered in `bootstrap/providers.php`. `C2` (Users) still pending promotion.
