...
class ApplicationAdminProductsHandlerBoundaryTest extends TestCase
{
    /**
     * Ensure admin product handlers do not return ORM models or paginator contracts.
     */
    public function test_admin_product_handlers_do_not_return_orm_or_paginator_types(): void
    {
        $handlers = $this->discoverAdminProductHandlerClasses();
        $this->assertNotEmpty($handlers, 'No admin product handlers found for architecture guardrail.');
     ...
     */
    private function discoverAdminProductHandlerClasses(): array
    {
        $directories = [
            [
                'path' => app_path('Application/Admin/Products/Commands'),
                'namespace' => 'App\\Application\\Admin\\Products\\Commands\\',
            ],
            [
                'path' => app_path('Application/Admin/Products/Queries'),
        ...
        foreach ($directories as $definition) {
            ...
            foreach (File::allFiles($handlersDirectory) as $file) {
                ...
                if (! class_exists($handlerClass)) {
                    $this->fail("Admin product handler class {$handlerClass} could not be autoloaded.");
                }
