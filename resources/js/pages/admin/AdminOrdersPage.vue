<template>
    <section class="grid">
        <div class="card">
            <div class="stack stack--between">
                <div>
                    ...
                    <p class="muted">Monitor order lifecycle and update statuses from one workspace.</p>
                </div>
            ...
            <div class="actions actions--top">
                <input v-model="filters.search" placeholder="Search by order number or email" :disabled="isLoading" />
                    ...
                    >
                        ...
                        <td>
                            <div class="status-stack">
                                <span class="status-chip" :class="orderStatusClass(order.status)">{{ order.status }}</span>
                        ...
                        <td>{{ formatDateTime(order.placed_at ?? order.created_at, { fallback: 'Unknown date' }) }}</td>
                        <td>
                            <button class="btn btn-muted" type="button" @click="selectOrder(order.id)">Details</button>
                        </td>
                    </tr>
                    </tbody>
                    <tbody v-else>
                    <tr>
                        <td colspan="6">
                            <div class="empty-state">
                                <p>{{ isLoading ? 'Loading orders...' : 'No orders match current filters.' }}</p>
                            </div>
                        </td>
                    </tr>
                    </tbody>
        ...
        <div class="card" v-if="selectedOrderDetail || isDetailLoading">
            ...
            <div class="stack stack--between">
                <div>
                    <h2 class="section-title">Order details: {{ selectedOrderDetail.order_number }}</h2>
                    <p class="muted">Customer: {{ selectedOrderDetail.email }}</p>
                </div>
                <div class="actions">
                    <span class="status-chip" :class="orderStatusClass(selectedOrderDetail.status)">{{ selectedOrderDetail.status }}</span>
                    <span class="status-chip" :class="paymentStatusClass(selectedOrderDetail.payment_status)">
                        {{ selectedOrderDetail.payment_status }}
                    </span>
                    <span class="status-chip" :class="shipmentStatusClass(selectedOrderDetail.shipment_status)">
            ...
            <div class="grid grid-3 actions--top">
                <article class="metric-card metric-card--soft">
                    ...
                    <strong class="metric-card__value">{{ formatPrice(selectedOrderDetail.total, selectedOrderDetail.currency) }}</strong>
                </article>
                <article class="metric-card metric-card--soft">
                ...
                <article class="metric-card metric-card--soft">
                    ...
                    <strong class="metric-card__value">{{ selectedOrderDetail.items.length }}</strong>
                </article>
            </div>
            ...
            <div class="actions actions--top">
                <button class="btn btn-primary" type="button" :disabled="currentDraft.saving" @click="updateSelectedOrderStatus">
            ...
            <div class="grid grid-2 actions--top">
                <article class="order-detail-box">
                    ...
                    <p class="muted">{{ formatAddress(selectedOrderDetail.billing_address) }}</p>
                </article>
                <article class="order-detail-box">
                    <h3>Shipping address</h3>
                    <p class="muted">{{ formatAddress(selectedOrderDetail.shipping_address) }}</p>
                </article>
            </div>
            ...
            <div class="table-wrap actions--top">
                <table class="table">
                    ...
                    <tbody>
                    <tr v-for="item in selectedOrderDetail.items" :key="`${selectedOrderDetail.id}-${item.sku}`">
                        <td>
                            <strong>{{ item.name }}</strong>
                            <p class="muted">{{ item.sku }}</p>
                        </td>
                        <td>{{ item.quantity }}</td>
                        <td>{{ formatPrice(item.unit_price, selectedOrderDetail.currency) }}</td>
                        <td>{{ formatPrice(item.total_price, selectedOrderDetail.currency) }}</td>
                    </tr>
                    </tbody>
                </table>
            </div>
